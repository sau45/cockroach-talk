        ...
        {activeMembers.map((member) => {
          ...
          return (
            <div
              ...
              className={cn(
                'group relative flex flex-col items-center p-2 rounded-brutal-md border-2 border-border bg-card shadow-brutal-dark-sm transition-all duration-200 cursor-pointer hover:-translate-y-0.5 hover:shadow-brutal hover:border-primary',
            ...
            >
              ...
              <div className="relative w-full aspect-[4/3] rounded-brutal-sm border-2 border-border bg-black/95 overflow-hidden shadow-inner flex items-center justify-center">
                {memberVideoEnabled ? (
                  hasLiveVideoTrack && stream ? (
                    <VideoChipMedia stream={stream} isSelf={isMe} />
                  ) : (
                    <Skeleton className="absolute inset-0 w-full h-full bg-card/90 flex flex-col items-center justify-center gap-1.5 border-0 z-10">
                      <Video className="h-4 w-4 text-primary animate-bounce" />
                      <span className="text-[9px] font-mono font-bold text-muted-foreground">Connecting camera...</span>
                    </Skeleton>
                  )
                ) : (
                  <UserAvatar
                ...
                {isTargetLead ? (
                  <div
                    title="Room Lead"
                    className="absolute top-1.5 left-1.5 flex items-center gap-1 text-[9px] font-mono font-bold text-accent-gold bg-black/85 border border-accent-gold/60 px-1.5 py-0.5 rounded shadow-sm z-10 pointer-events-none"
                  >
                  ...
                  <div
                    ...
                    className="absolute top-1.5 left-1.5 flex items-center gap-1 text-[9px] font-mono font-bold text-accent-gold bg-black/85 border border-accent-gold/40 px-1.5 py-0.5 rounded shadow-sm z-10 pointer-events-none"
                  >
                    <Shield className="h-2.5 w-2.5 text-accent-gold" />
                    <span>MOD</span>
                  </div>
                ) : null}
                ...
                <div
                  className="absolute top-1.5 right-1.5 flex items-center gap-1 z-20"
                ...
                >
                  {canMuteThisMember && (
                    <button
                      ...
                      title="Mute speaker"
                      className="p-1 rounded bg-black/80 border border-white/20 text-muted-foreground hover:text-accent-coral transition-colors hover:bg-black"
                    >
                  ...
                  {canDemoteThisMember && (
                    <button
                      ...
                      className="p-1 rounded bg-black/80 border border-white/20 text-muted-foreground hover:text-destructive transition-colors hover:bg-black"
                    >
                ...
                <div className="absolute bottom-1.5 right-1.5 flex items-center gap-1 z-10 pointer-events-none">
                  {!member.isMuted ? (
                    <span
                      title="Microphone Open"
                      className="p-1 rounded-full bg-emerald-600 text-white shadow-sm flex items-center justify-center border border-emerald-400/80 ring-2 ring-emerald-500/40 animate-pulse"
                    >
                    ...
                    <span
                      title="Muted"
                      className="p-1 rounded-full bg-destructive text-white shadow-sm flex items-center justify-center border border-black/50"
                    >
                  ...
                  {member.isScreenSharing && (
                    <span
                      ...
                      className="p-1 rounded-full bg-primary text-primary-foreground shadow-sm flex items-center justify-center animate-pulse border border-black/50"
                    >
              ...
              <div className="w-full text-center mt-1.5 px-0.5 truncate flex items-center justify-center gap-1">
                <p className={cn("font-bold text-xs truncate font-mono", memberPalette.textClass)}>
                  {member.name}
                </p>
                {isMe && (
                  <span className="text-[9px] font-mono font-bold px-1 rounded bg-primary/20 text-primary border border-primary/30 shrink-0">
                    YOU
                  </span>
                )}
              </div>
